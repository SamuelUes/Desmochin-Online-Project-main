import { cardLabel, sameCardSet, sortMeld } from "./cards.js";

export function playedCount(player) {
...
export function findMeldOptions(player, candidate) {
    const options = [];
...
export function optionMatchesSelection(option, selectedCards) {
    return sameCardSet(option.cardsFromHand, selectedCards);
}

export function hasForcedClaim(options) {
    return options.some(option => option.forced);
}

export function isCardUsefulToPlayer(player, card) {
    return findMeldOptions(player, card).length > 0;
}

export function getForcedMeldOption(player, card) {
    const options = findMeldOptions(player, card);
    return options.find(option => option.forced) ?? null;
}

export function findHandMeldOptions(player) {
    const options = [];
    for (const group of cardCombinations(player.hand, 3, Math.min(6, player.hand.length))) {
        ...
        options.push({
            ...
            resultCards,
            label: `Bajarse con ${meldLabel(resultCards)}`,
...
export function findExtensionTargets(player, card) {
    return player.melds
        .map((meld, meldIndex) => {
            ...
            return {
                ...
                cardsFromHand: [card],
                resultCards,
                label: `Agregó ${cardLabel(card)} a ${meldLabel(meld.cards)}`,
...
export function canRemoveFromMeld(meld, cardsToRemove) {
...
export function isValidStrategyMeld(stagedCards) {
...
function addExtensionOptions(options, player, candidate) {
    player.melds.forEach((meld, meldIndex) => {
        ...
        options.push({
            ...
            cardsFromHand: [],
            resultCards,
            label: `Agregar ${cardLabel(candidate)} a ${meldLabel(meld.cards)}`,
            forced: true,
...
function addSetOptions(options, player, candidate) {
    ...
    for (let size = 2; size <= Math.min(3, matching.length); size += 1) {
        ...
        options.push({
            ...
            resultCards,
            label: `Formar cuerpo de ${candidate.label}`,
    ...
    for (let start = 1; start <= 11; start += 1) {
        for (let end = start + 2; end <= Math.min(13, start + 9); end += 1) {
            ...
            options.push({
                ...
                cardsFromHand: handCards,
                resultCards,
            label: `Formar escalera ${meldLabel(resultCards)}`,
...
function addTrasmocharOptions(options, player, candidate) {
    ...
    for (const group of entryCombinations(pool, 2, Math.min(5, pool.length))) {
        ...
        options.push({
            ...
            borrowedCards,
            resultCards,
            label: `Trasmochar con ${meldLabel(resultCards)}`,
...
function meldLabel(cards) {
    return sortMeld(cards).map(cardLabel).join(" ");
}

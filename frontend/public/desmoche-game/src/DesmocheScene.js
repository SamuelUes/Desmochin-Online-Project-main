...
export class DesmocheScene extends Phaser.Scene {
    ...
    create() {
        ...
        document.getElementById("reset-btn").addEventListener("click", () => this.handleResetButton());
        ...
        if (!this.multiplayerData && localStorage.getItem("desmoche_roomCode")) {
            this.multiplayerData = {
                gameState: null,
                multiplayer: true
            };
    ...
    handleResetButton() {
        if (this.isMultiplayerGame()) {
            window.top.location.href = "/salas";
            return;
        }
        this.resetGame();
    }
    ...
    initMultiplayerGame(gameState) {
        ...
        this.renderAll();
        this.showMessage('Sincronizando partida...');
    }
        ...
        import('./socketClient.js').then(({ default: socket }) => {
            ...
            const joinCurrentRoom = () => {
                if (roomCode && userId && username) {
                    ...
                    socket.emit('game:getSnapshot', { roomCode, userId });
                }
            };
            ...
            socket.on('gameSnapshot', ({ gameState }) => {
                this.applyServerSnapshot(gameState);
            });

            socket.on('gameError', ({ message }) => {
                this.showMessage(message ?? 'No se pudo realizar la accion.');
            });
    ...
    applyServerSnapshot(gameState) {
        if (!gameState) return;

        const myUserId = localStorage.getItem('desmoche_userId');
        const serverPlayers = gameState.players ?? [];
        const myIndex = Math.max(0, serverPlayers.findIndex(p => p.userId === myUserId));
        const reordered = [
            ...serverPlayers.slice(myIndex),
            ...serverPlayers.slice(0, myIndex)
        ];
        const myHand = sortCards((gameState.myHand ?? []).filter(Boolean));

        this.players = reordered.map((player, index) => ({
            name: player.username,
            isHuman: index === HUMAN_INDEX,
            hand: index === HUMAN_INDEX
                ? myHand
                : gameState.phase === 'gameOver' && Array.isArray(player.hand)
                    ? sortCards(player.hand.filter(Boolean))
                    : Array.from({ length: player.cardCount ?? 0 }, (_, cardIndex) => ({
                        id: `hidden-${player.userId}-${cardIndex}`,
                        hidden: true
                    })),
            melds: player.melds ?? [],
        ...
        this.deck = Array.from({ length: gameState.deckCount ?? gameState.mainDeck ?? 0 }, (_, index) => ({
            id: `deck-${index}`
        }));
        this.discardPile = gameState.discardPile ?? [];
        this.exposedCard = gameState.exposedCard ?? null;
        this.currentPlayer = Math.max(0, this.players.findIndex(player => player.userId === gameState.currentTurn));
        this.turnCanDraw = !!gameState.turnCanDraw && !!gameState.canAct;
        this.hasDrawnThisTurn = !!gameState.hasDrawnThisTurn;
        this.dealing = false;
        this.gameActive = gameState.phase !== 'gameOver';

        if (gameState.phase === 'gameOver') {
            this.phase = 'gameOver';
            this.currentPlayer = Math.max(-1, this.players.findIndex(player => player.userId === gameState.winnerUserId));
            const resetButton = document.getElementById('reset-btn');
            resetButton.textContent = 'Volver a salas';
        ...
        if (gameState.phase === 'gameOver') {
            // The phase was already applied above.
        } else if (gameState.phase === 'cambio') {
            this.phase = gameState.canAct ? 'humanCambio' : 'thinking';
        } else if (gameState.phase === 'pay') {
            this.phase = gameState.canAct ? 'humanPay' : 'thinking';
        } else if (gameState.phase === 'turn') {
            this.phase = gameState.canAct ? 'humanTurn' : 'thinking';
        } else {
            this.phase = 'thinking';
        }
        ...
        this.clearAllTimers();
        this.renderAll();
        this.showMessage(gameState.message ?? 'Partida sincronizada.');
    }
    ...
    finishHumanActionWindow() {
        if (this.isMultiplayerGame() && this.phase === "humanTurn") {
            if (!this.hasDrawnThisTurn) {
                this.showMessage("Debes sacar una carta antes de terminar.");
                return;
            }
    ...
    commitDesmocharSelection() {
        ...
        if (this.isMultiplayerGame()) {
            this.emitMultiplayerAction({
                type: 'desmochar',
                cardIds: this.selectedActionCards.map(card => card.id),
                meldCards: this.selectedMeldEntries.map(entry => ({
    ...
    finishHumanCambio(card) {
        if (this.isMultiplayerGame()) {
            if (!card) return;
            this.socket?.emit('game:selectCambio', {
                roomCode: localStorage.getItem('desmoche_roomCode'),
                userId: localStorage.getItem('desmoche_userId'),
                cardId: card.id
            });
    ...
    renderMeldsVertical(playerIndex, player, layout) {
        ...
        const rowGap = 94;
        const panelH = this.meldPanelHeight(layout);
        const centerX = seat === 1 ? base.x - 8 : base.x + 8;
    ...
    renderCommandButtons() {
        ...
        if (this.isHumanActionPhase()) {
            ...
            if (!this.isMultiplayerGame() || this.hasDrawnThisTurn) {
    ...
    handleActionDrop(entry, x, y) {
        if (this.isMultiplayerGame()) {
            this.showMessage("Selecciona cartas y confirma la accion con los botones.");
            this.renderAll();
            return;
        }
    ...
    emitMultiplayerAction(action) {
        this.socket?.emit('game:submitAction', {
            roomCode: localStorage.getItem('desmoche_roomCode'),
            userId: localStorage.getItem('desmoche_userId'),
            action
        });
    ...
    createCardView(card, x, y, w, h, options = {}) {
        ...
        if (faceUp && card && !card.hidden && card.label && card.suitSymbol) {
    ...
    visualSeat(playerIndex) {
        if (this.players?.length === 2 && playerIndex === 1) return 2;
        return playerIndex;
    }
    ...
    playerDropArea(playerIndex) {
        const seat = this.visualSeat(playerIndex);
        const { width, height } = this.scale;
        if (seat === 1) return { x: width - 138, y: height / 2 - 200, w: 132, h: 400 };
        if (seat === 2) return { x: width / 2 - 220, y: 20, w: 440, h: 128 };
        if (seat === 3) return { x: 6, y: height / 2 - 200, w: 132, h: 400 };
